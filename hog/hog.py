...
def is_prime(num):
    ...
    assert num < 100 and type(num) == int and num > 0,'Impossible scores'
    if num == 1:
        return False
    trial_num, k = int(pow(num, 0.5)), 2
    while k <= trial_num:
        if (num // k) * k == num:
            return False
        k = k + 1
...
def next_prime(num):
    """ return the next prime of the given prime"""
    assert is_prime(num), 'It is not a prime.'
    num = num + 1
